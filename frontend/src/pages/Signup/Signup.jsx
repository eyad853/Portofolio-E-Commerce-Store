...
const Signup = () => {
    ...
    const handleFileChange = (e) => {
      ...
      const handleTryAsAdmin = async () => {
        setLoadings(true)
        try {
            const res = await axios.post('http://localhost:8000/tryAsFakeAdmin',{} , {withCredentials:true});
            if(!res.data.error){
                navigate('/home')
            }
            setLoadings(false)
        } catch (error) {
          console.error('Error trying as admin:', error);
    ...
    return (
        <div className='w-screen p-4 lg:p-10 text-white min-h-screen flex justify-center items-center'>
                            ...
                            <div className='w-full h-14 flex flex-col '>
                                ...
                                <div className='rounded-md h-full bg-gray-500/30 flex'>
                                    ...
                                    <input
                                        type="text"
                                        value={username}
                                        name='username' // Important: `name` attribute
            ...
            {loadings&&(
                <div className="fixed inset-0 bg-gray-300/50 flex justify-center items-center">
                    <div className="w-12 h-12 lg:w-90 lg:h-90 rounded-full border-y-2 border-blue-600 animate-spin"></div>
                </div>
            )}
        </div>
    );
